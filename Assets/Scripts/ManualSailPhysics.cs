...
public class ManualSailPhysics : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.RotateAround(mast.transform.position, transform.up,  Mathf.Sign(-dotRight) * WindManager.instance.windMagnitude * (1- dotForward) * Time.deltaTime * windAttachmentFactor);
        float rad = Mathf.Abs(transform.localRotation.y * Mathf.Deg2Rad * 10);
        //sail.material = defaultMat;
        if (rad > rope)
        {
            transform.RotateAround(mast.transform.position, transform.up,  Mathf.Sign(dotRight) * WindManager.instance.windMagnitude * (1- dotForward) * Time.deltaTime * adjustmentFactor * windAttachmentFactor);
