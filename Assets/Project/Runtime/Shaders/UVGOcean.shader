	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float4 fetchOpaqueVal199 = float4( SHADERGRAPH_SAMPLE_SCENE_COLOR( ( (ase_grabScreenPosNorm).xyzw + float4( ( _RefractAmount * Normals148 ) , 0.0 ) ).xy ), 1.0 );
				float4 clampResult201 = clamp( fetchOpaqueVal199 , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 Refraction204 = clampResult201;
				float temp_output_92_0 = ( _TimeParameters.x * _WaveSpeed );
				float4 WaveTileUv123 = ( ( worldSpaceTile127 * float4( _WaveStretch, 0.0 , 0.0 ) ) * _WaveTile );
				float2 panner95 = ( temp_output_92_0 * _WaveDirection + WaveTileUv123.xy);
				float simplePerlin2D99 = snoise( panner95*_WaveScale );
				...
				float screenDepth200 = LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth200 = abs( ( screenDepth200 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _Depth ) );
				float clampResult202 = clamp( distanceDepth200 , 0.0 , 1.0 );
				float depth203 = clampResult202;
				float4 lerpResult188 = lerp( Refraction204 , albedo157 , depth203);
				...
				float2 temp_cast_8 = ((normalLightDir17*_RampScale + _RampScale)).xx;
				float4 shadow62 = ( albedo157 * tex2D( _ToonRamp, temp_cast_8 ) );
				...
				float3 Albedo = lerpResult188.rgb;
...
/*ASEBEGIN
Version=18900
291;73;1016;601;189.444;234.5033;1.223579;True;False
...
ASEEND*/
//CHKSM=8B2FA866C21974503AB80749EB1E127D0DBC983B